				...
				<div class="row">
					<div class="col-md-9 paddingright0">
						<div class="score-card margintop15">
							<div class="recent">
								<?php 
									$playbowler = $_GET['playbowlerid'];
								 ?>
								<?php $singlerecent = get_single_bowler($playbowler);
								...
								<table class="table">
									...
									<tr>
										<td><?php echo get_player_name($recent['bowlerid']); ?></td>
										<td><input type="text" name="Madins" value="<?php echo $recent['Madins']; ?>" class="form-control"></td>
										<td>
											<select class="form-control" name="playstatus">
												<option value="1" <?php $recent['playstatus'] == '0' ? ' selected="selected"' : '';?>>Yes</option>
												<option value="0" <?php $recent['playstatus'] == '1' ? ' selected="selected"' : '';?>>No</option>
											</select>
										</td>
									</tr>
									<tr >
										<td colspan="4"><input type="submit" name="" class="edit-bowler-single"></td>
									</tr>
								</table>
								<?php endforeach; ?>
							</div>
							...
							$(document).ready(function(){
								$('.edit-bowler-single').click(function(event){
									event.preventDefault();
									var playbowlerid = <?php echo $_GET['playbowlerid']; ?>;
									var Madins = $('input[name="Madins"]').val();
									var playstatus = $('select[name="playstatus"]').val();
									$.post("http://www.livecricketinfo.com/action.php",
									{	
										action:"editbowlerfullscore",
										playbowlerid:playbowlerid,
										Madins:Madins,
